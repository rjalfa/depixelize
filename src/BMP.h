// From https://github.com/sol-prog/cpp-bmp-images ref:04427f27fe047bf5b1a2811a8d0cb0f9eface626
...
struct BMP {
    ...
    void get_pixel(uint32_t x0, uint32_t y0, uint8_t& B, uint8_t& G, uint8_t& R, uint8_t& A) {
        if (x0 > (uint32_t)bmp_info_header.width || y0 > (uint32_t)bmp_info_header.height) {
            throw std::runtime_error("The point is outside the image boundaries!");
        }
